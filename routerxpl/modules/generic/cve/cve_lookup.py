...
LOCAL vs PHYSICAL) and indicates which ones have RouterXPL-Forge exploit
...
class Exploit(Exploit):
    __info__ = {
        ...
        "description": "Queries the embedded CVE database for known vulnerabilities "
                       ...
                       "PHYSICAL. Lists available RouterXPL-Forge exploit modules. "
    ...
    def run(self):
        ...
        if remote_exploitable:
            print_success("=== EXPLOITABLE by RouterXPL-Forge ({}) ===".format(len(remote_exploitable)))
            print_status("")
        ...
        if non_remote and self.show_physical:
            ...
            print_info("  (RouterXPL-Forge cannot exploit these remotely)")
            print_status("")
