/*
 * test_inject_dylib.c — Tests libload_inject_dylib (ptrace + remote dlopen)
 *
 * Requires ptrace access (root or Yama scope 0).
 *
 * Usage: test_inject_dylib <target_binary> <payload.so>
 */
...
{
    if (argc < 3) {
        fprintf(stderr, "usage: %s <target> <payload.so>\n", argv[0]);
        return 1;
    }
    ...
    printf("=== libload_inject_dylib (ptrace + dlopen) ===\n");
