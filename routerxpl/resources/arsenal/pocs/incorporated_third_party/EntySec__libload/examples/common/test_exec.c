/*
 * test_exec.c — Tests libload_exec (fork + in-memory execution)
 *
 * Usage: test_exec [path_to_executable]
 */
...
{
    const char *path = argc > 1 ? argv[1] : "testexec";

    printf("=== libload_exec test (%s) ===\n", path);
