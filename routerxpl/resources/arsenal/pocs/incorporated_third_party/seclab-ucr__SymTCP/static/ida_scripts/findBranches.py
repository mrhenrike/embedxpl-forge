...
file0 = AskFile(0, "*.txt", "Select previously unfinished file")
fd0 = open(file0, 'r')
...
for line in fd0:
    if line.startswith("[Start]"):
        line = line.rstrip()
        parts = line.split()
        c = (parts[1], int(parts[3], 16), int(parts[5], 16))
        found[c] = []
...
def find_paths(func_name, start_ea, end_ea):
    ...
    def _find_paths_forward(curr_bb, level):
        ...
        if succ_num == 0:
            ...
            br_mnem = GetMnem(br_ea)
            br_opval = GetOperandValue(br_ea, 0)
            br_optype = GetOpType(br_ea, 0)
            fd1.write("[TEST]%s %s (%s)\n" % (br_mnem, br_opval, br_optype))
            assert br_optype == 7
    ...
    # SAVE ALL THE BRANCHES THAT ONLY ZERO OR ONE CHILD IS VISITED
    #print(branches)
