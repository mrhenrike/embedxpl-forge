...
def lookup(mac: str, *, online: bool = True) -> str:
    """Look up vendor for a MAC address (normalized to RouterXPL convention).
...
def normalize_vendor(vendor_raw: str) -> str:
    """Normalize a raw vendor string to RouterXPL folder convention.
    ...
    for api in _ONLINE_APIS:
        ...
        try:
            req = urllib.request.Request(
                url,
                headers={"User-Agent": "RouterXPL-Forge/0.2.0"},
            )
