...
"""APT Group Attack Catalog — maps real-world threat actors to RouterXPL modules.

Provides a structured catalog of Advanced Persistent Threat (APT) groups
that target network devices (routers, switches, IoT) covered by RouterXPL.
...
logger = logging.getLogger("routerxpl.apt_catalog")
...
@dataclass
class APTGroup:
    """APT group profile with associated attacks reproducible by RouterXPL."""
    id: str
    name: str
    aliases: List[str]
    country: str
...
def _build_catalog() -> None:
    """Populate the APT catalog with known groups targeting RouterXPL-covered devices."""
